<?php
    ...
    $sql = "SELECT * FROM phongban LIMIT $start_from, $num_per_page";

    $result = mysqli_query($connection, $sql);
    $listDepartments = mysqli_fetch_all($result, MYSQLI_ASSOC);
...
<body>
    <div id="main">
        <h4 class="center mt-16">Danh sách phòng ban</h4>
        <?php if (isset($_SESSION['login'])) : ?>
            <div class="controls">
                <a type="button" href="themPB.php" class="btn btn-primary mt-16 mr-44">Thêm</a>
            </div>
        <?php endif; ?>
        ...
        <div class="container mt-16 table-responsive">
            <?php if (count($listDepartments) == 0) : ?>
                <h4 class="center">Chưa có phòng ban trong hệ thống!</h4>
            <?php else : ?>
                <table class="table table-striped table-hover table-bordered">
                    <thead class="thead-dark">
                        <tr>
                            ...
                            <th>Tên phòng ban</th>
                            <th>Mô tả</th>
                            <th>Xem danh sách nhân viên</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php foreach ($listDepartments as $department) : ?>
                            <tr>
                                <td><?php echo htmlspecialchars($department['IDPB']); ?></td>
                                <td><?php echo htmlspecialchars($department['TenPB']); ?></td>
                                <td><?php echo htmlspecialchars($department['MoTa']); ?></td>
                                <td>
                                    <a href="xemthongtinNVPB.php?id=<?php echo htmlspecialchars($department['IDPB']) ?>&tenpb=<?php echo htmlspecialchars($department['TenPB']) ?>">Xem</a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                ...
                <ul class="pagination justify-content-end">
                    <?php 
                        $pagination_query = "SELECT * FROM phongban;";
